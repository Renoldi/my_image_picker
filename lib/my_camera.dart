          ...
          Expanded(
            child: Container(
              ...
              child: Align(
                ...
                child: CameraPreview(widget.controller.value.cameraController!),
              ),
            ),
          ),
          Container(
...
class CameraComponentController extends ValueNotifier<CameraComponentValue> {
  CameraComponentController({CameraComponentValue? value})
    : super(value ?? CameraComponentValue());
  ...
  void initCamera({CameraDescription? cameraDescription}) {
    ...
    availableCameras().then((availableCamera) {
      ...
      value.cameraController?.lockCaptureOrientation(
        DeviceOrientation.portraitUp,
      );
      value.cameraController?.initialize().then((camera) {
  ...
  void changeCamera() {
    if ((value.cameraDescriptions?.length ?? 0) > 1) {
      int indexCamra = (value.cameraDescriptions)!.indexOf(
        value.selectedCamera!,
      );
      if ((indexCamra + 1) <= (value.cameraDescriptions ?? []).length - 1) {
        value.cameraController!.dispose().then((dispose) {
          initCamera(
        ...
        value.cameraController!.dispose().then((dispose) {
          initCamera(cameraDescription: value.cameraDescriptions?.first);
        });
      }
    }
  }
  ...
  void takePicture() {
    value.cameraController?.takePicture().then((image) {
...
enum CameraComponensStates { onInitializeCamera, onOpenedCamera, onLoadedImage }
